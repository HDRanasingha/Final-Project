...
router.get('/all', authenticateUser, async (req, res) => {
    try {
        const flowers = await Flower.find({ growerId: req.growerId }).populate("growerId", "firstName lastName");
        res.status(200).json(flowers);
    } catch (error) {
        res.status(500).json({ success: false, message: "Error fetching flowers", error: error.message });
    }
...
router.get('/:id', authenticateUser, async (req, res) => {
    try {
        const flower = await Flower.findOne({ _id: req.params.id, growerId: req.growerId }).populate("growerId", "firstName lastName");
...
router.delete('/delete/:id', authenticateUser, async (req, res) => {
    try {
        const flower = await Flower.findOne({ _id: req.params.id, growerId: req.growerId });
        if (!flower) return res.status(404).json({ success: false, message: "Flower not found" });
